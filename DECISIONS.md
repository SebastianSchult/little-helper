...
| 2026-06-17 | Accessibility-Gate im Onboarding weich (Weiter immer aktiv) | `AXIsProcessTrusted()` bei ad-hoc signierten Apps nicht live nach Permission-Grant — harter Gate blockiert User ohne erkennbaren Grund | User kann ohne Accessibility fortfahren; Workaround: "Erneut prüfen"-Button | Accepted |
| 2026-06-17 | Erster Modell-Download passiert beim ersten Hotkey, nicht beim Start | WhisperKit lädt nur wenn `prepare()` aufgerufen wird; Pre-load triggert Download falls kein Cache | Erster Start langsam (~150 MB); alle Folgestarts schnell | Known Constraint |
